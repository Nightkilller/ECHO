...
class WindowPositionManager {
    ...
    /// Returns true if the app has Automation permission for Finder.
    static func hasFinderAutomationPermission() -> Bool {
        let appleScript = "tell application \"Finder\" to get name"
        guard let script = NSAppleScript(source: appleScript) else { return false }
        var error: NSDictionary?
        _ = script.executeAndReturnError(&error)
        if let err = error {
            // Error number -1743 is "errAEEventNotAllowed" (User declined automation permission)
            if let errNumber = err[NSAppleScript.errorNumber] as? Int, errNumber == -1743 {
                return false
            }
        }
        return true
    ...
    /// Triggers the system prompt for Finder Automation.
    static func requestFinderAutomationPermission() {
        let appleScript = "tell application \"Finder\" to get name"
    ...
    // MARK: - Desktop Folder Permission

    /// Returns true if the app has access to the user's Desktop folder.
    static func hasDesktopFolderPermission() -> Bool {
    ...
    /// Triggers the system Files & Folders prompt for Desktop access.
    /// Falls back to an NSOpenPanel if the automatic TCC prompt doesn't appear.
    static func requestDesktopFolderPermission() {
        let desktopPath = ("~/Desktop" as NSString).expandingTildeInPath
        
        // First attempt: try reading the directory, which triggers the TCC prompt
        // when NSDesktopFolderUsageDescription is in Info.plist
        do {
            _ = try FileManager.default.contentsOfDirectory(atPath: desktopPath)
            // If we get here, permission is already granted
            return
        } catch {
            // Permission denied — try NSOpenPanel fallback
        }
        
        // Second attempt: use NSOpenPanel to let the user explicitly grant access
        // to their Desktop folder
        let openPanel = NSOpenPanel()
        openPanel.title = "Grant Desktop Folder Access"
        openPanel.message = "Echo needs access to your Desktop folder to open and manage folders. Please select your Desktop folder."
        openPanel.canChooseDirectories = true
        openPanel.canChooseFiles = false
        openPanel.allowsMultipleSelection = false
        openPanel.canCreateDirectories = false
        openPanel.directoryURL = URL(fileURLWithPath: desktopPath)
        openPanel.prompt = "Grant Access"
        
        openPanel.begin { response in
            if response == .OK {
                print("🔑 User granted Desktop folder access via NSOpenPanel")
            } else {
                // Open System Settings as a last resort
