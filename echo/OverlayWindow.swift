...
struct BlueCursorView: View {
    ...
    var body: some View {
        ZStack {
            ...
            Triangle()
                ...
                .opacity(buddyIsVisibleOnThisScreen && companionManager.allPermissionsGranted && (companionManager.voiceState == .idle || companionManager.voiceState == .responding) ? cursorOpacity : 0)
            ...
            BlueCursorWaveformView(audioPowerLevel: companionManager.currentAudioPowerLevel)
                .opacity(buddyIsVisibleOnThisScreen && companionManager.allPermissionsGranted && companionManager.voiceState == .listening ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
            ...
            BlueCursorSpinnerView()
                .opacity(buddyIsVisibleOnThisScreen && companionManager.allPermissionsGranted && companionManager.voiceState == .processing ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                .animation(.easeIn(duration: 0.15), value: companionManager.voiceState)
        ...
        .onChange(of: companionManager.detectedElementScreenLocation) { newLocation in
            ...
            // Check if the target is on THIS screen. Use a tolerance for CGRect
            // comparison since floating-point display coordinates may differ slightly.
            let displayMid = CGPoint(x: displayFrame.midX, y: displayFrame.midY)
            let screenMid = CGPoint(x: screenFrame.midX, y: screenFrame.midY)
            let isOnThisScreen = screenFrame.contains(displayMid)
                || (abs(displayMid.x - screenMid.x) < 1 && abs(displayMid.y - screenMid.y) < 1)

            print("🎯 BlueCursorView onChange — target: \(screenLocation), displayFrame: \(displayFrame), screenFrame: \(screenFrame), isOnThisScreen: \(isOnThisScreen)")

            guard isOnThisScreen else {
                return
            }
    ...
    private func animateBezierFlightArc(
        ...
        let newTimer = Timer(timeInterval: frameInterval, repeats: true) { timer in
            currentFrame += 1
        ...
        RunLoop.main.add(newTimer, forMode: .common)
        self.navigationAnimationTimer = newTimer
    }
